...
public class PasswordResetService {
    ...
    public Map<String, Object> requestForgotPassword(String email) {
        ...
        String fallbackResetLink = null;

        if (userOpt.isPresent()) {
            AppUser user = userOpt.get();
            if (isInCooldown(user.getId())) {
                log.info("Password reset request throttled by cooldown.");
                return genericForgotResponse(null);
            }
            ...
            if (demoResetLinkEnabled) {
                ...
                fallbackResetLink = resetLink;
            } else {
                try {
                    passwordResetMailService.sendPasswordResetEmail(user.getEmail(), resetLink);
                } catch (MailException ex) {
                    // Keep a generic API response, but provide a direct fallback link to unblock legitimate users.
                    log.warn("Password reset email was not delivered. reason={}", ex.getClass().getSimpleName());
                    fallbackResetLink = resetLink;
                }
            }
            log.info("Password reset requested (email redacted).");
        }

        return genericForgotResponse(fallbackResetLink);
    }

    private Map<String, Object> genericForgotResponse(String resetLink) {
